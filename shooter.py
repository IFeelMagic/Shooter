...
#def affiche_cible():
    #print('Nombre de cible :', world['nbr_cible'],',', 'Nombre de cible actuelle :', len(targets))

def move(direction:str, targets:list):
    if direction=='droit':
        for t in targets:
            t['rectangle'][0] += 1 
    if direction=='gauche':
...
def add_target(targets):
    ...
    #print ('Nombre de cible', world['nbr_cible'], ',', 'Cible en', x, y,',', 'Cible actuellement présente :', len(targets))
    targets.append(
        {
            'rectangle': [
...
def update_display(scene, world, targets):
    ...
    for explosion in explosions:
        ...
        pygame.draw.polygon(scene, (200, 200 * shade, 0), explosion['points_current'])  # type: ignore
